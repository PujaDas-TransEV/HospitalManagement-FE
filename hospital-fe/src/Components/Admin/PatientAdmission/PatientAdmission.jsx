...
const PatientAdmissionPage = () => {
  ...
  const fetchBedDetails = async () => {
    try {
      const response = await axios.get('http://localhost:5000/ops/getbeddetails');
      if (response.data && Array.isArray(response.data.data)) {
  ...
  const handlePatientAdmission = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/ops/patientadmit', formData, {
        headers: {
  ...
  const handleSearchByAdmissionId = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/ops/getbeddetails', formData);
