...
const AdminLabTestPage = () => {
  ...
  const fetchAllReports = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:5000/ops/labreportall');
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:5000/labreport/labbyid', formData);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/ops/labupdate', formData);
